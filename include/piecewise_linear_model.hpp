...
private:
    ...
    // 有一个key的列表，已经排序好了，例如[1, 4, 7, 9, 20, 23, 28, 30, 31, 33]
    // 然后x是key，y是key的位置，比如 (1, 0), (4, 1), (7, 2), (9, 3), (20, 4), (23, 5), (28, 6), (30, 7), (31, 8), (33, 9)
    // 这个模型的作用就是，给定一个x，找到一个y，使得y是一个线性函数，这个线性函数是一个分段线性函数，这个函数的斜率是一个常数
    // 通过给定的key 预测 key的位置
    struct Point {
        X x{};
        Y y{};
...
public:
    ...
    bool add_point(const X &x, const Y &y) {
        // std::cout << "add_point: x=" << x << ", y=" << y << std::endl;
