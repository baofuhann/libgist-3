 ...
 *   Model nodes use a linear regression model (or other simple models) to predict 
 *   the position of a key and map the prediction result to its child node.
 *   The information stored in model nodes includes model parameters (e.g., slope and intercept),
 *   which are used for quickly locating the target leaf node.
 *
 * - Data nodes, sometimes referred to as leaf nodes (equivalent to leaf nodes
 * of a B+ Tree)
 *   Data nodes store the actual key-value pairs, similar to where data is stored in traditional indexes.
 */
...
class AlexModelNode : public AlexNode<T, P> {
 public:
  ...
  // Return the corresponding child node pointer
...
class AlexDataNode : public AlexNode<T, P> {
 public:
  ...
  /*
  * If sampling is enabled (use_sampling = true), train the model using a subset of the data.
  * By default, the model is built using all data points.
  */
