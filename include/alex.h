...
//      int max_data_node_slots = (1 << 15) / sizeof(V);
  ...
  struct Stats {
    ...
    int num_expand_and_scales = 0; // 扩展和缩放次数
    int num_expand_and_retrains = 0; // 扩展和重新训练次数
    int num_downward_splits = 0;  // 向下分裂次数
    int num_sideways_splits = 0;  // 横向分裂次数
    int num_model_node_expansions = 0; // 模型节点扩展次数
    int num_model_node_splits = 0; // 模型节点分裂次数
    long long num_downward_split_keys = 0; // 向下分裂的键数
    long long num_sideways_split_keys = 0; // 横向分裂的键数
    long long num_model_node_expansion_pointers = 0; // 模型节点扩展指针数
    long long num_model_node_split_pointers = 0; // 模型节点分裂指针数
    mutable long long num_node_lookups = 0; // 节点查找次数
    mutable long long num_lookups = 0;  // 查找次数
    long long num_inserts = 0; // 插入次数
    double splitting_time = 0; // 分裂时间
    double cost_computation_time = 0; // 成本计算时间
  };
  Stats stats_;
 ...
 // 用于资源释放，文件操作，数据库连接等
  ...
  // explicit防止隐式类型转换, 只允许显式调用构造函数
  // 定义一个Person类, 有一个构造函数Person(int age), 如果不加explicit, 则可以这样调用: Person p = 10;
  // 如果加了explicit, 则只能这样调用: Person p(10);
  ...
  // Node& ref = node;     // 引用声明（& 在后）
  // Node* ptr = &node;    // 指针声明, 取地址（& 在前）
  // 在 C++ 中，函数参数通常有两种方式:传值和传引用。
  // 传值是指在调用函数时，将实参的值复制一份传递给形参，而传引用是指在调用函数时，将实参的地址传递给形参，形参通过这个地址对实参进行操作。
  ...
  typedef struct {
    char node_type; // 节点类型
    char empty_byte_count; // 空字节计数
    int children_number; // 子节点数
    double slope; // 斜率
    double intercept; // 截距
    // Need to add other stats later
    double cost; // 成本
    int level; // 节点层级
    uint8_t duplication_factor_; // 重复因子
  } ModelHeaderOnDisk;
...
//		    std::cout << "small" << std::endl;
  ...
  void create_root_node() {
    root_node_ = new (model_node_allocator().allocate(1)) 
...
// node's parent.
 ...
 public:
  ...
  void bulk_load(const V values[], int num_keys) {
    std::cout << "=== aaaaa Starting ALEX Bulk Load Process ===" << std::endl;
...
//                      TraversalPathDisk parent = traversal_path.back();
 ...
 private:
  ...
  model_node_type* split_downwards(
    ...
    /*
     * - 磁盘存储（disk-resident）：整个索引结构存储在磁盘上（例如 HDD 或 SSD）。
     * 所有的查找和插入操作都需要从磁盘加载相关数据，因而涉及磁盘 I/O 操作。
     * - 混合存储（hybrid）：将部分索引（如内部节点）固定存储在内存中，而数据节点仍存储在磁盘上。
     * 这种模式通过减少磁盘 I/O 的频率来提升性能。
     */
